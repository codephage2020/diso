...
> **This file is the law; `output-template.md` is the implementation.** Every rule here is already encoded in the template's stylesheet — copy CSS from there, never retype it from here. This file exists to tell you *which* class to reach for and *why*, and to carry the rules (SVG, above all) that no stylesheet can express.
...
The full `:root` block lives in `output-template.md` — copy it from there. This table says what each token is *for*; the closed set below is also what `scripts/check.py` enforces, so a hex outside it fails the build.
...
The `--serif` / `--mono` stacks are in the template's `:root`. What they encode:
...
Every class below is already styled in `output-template.md`. Take the markup from there; obey the rule here.
...
| `summary:focus-visible` | The page's only interactive elements are `<summary>` and TOC anchors; both keep a visible focus ring |
...
Nothing in this section is expressible as CSS, so it lives here in full. Ready-made snippets are in `output-template.md`.
...
Long pieces may alternate **at the block level** between parchment `#f5f4ed` and near-black celadon `#1B2E2E` — the strongest contrast device available.
